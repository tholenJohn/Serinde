 ...
 * @private
 */
const buildLogMessage_ = function (...var_args) {
    let message = '';
...
const doubleToIEEE754String = function (v) {
    ...
    let bias = (1 << (ebits - 1)) - 1, s, e, f, ln, i, bits, str;
 ...
 * @private
 */
const INTEGER_REGEXP_ = new RegExp('^-?\\d{1,10}$');
/**
...
const parseDatabaseURL = function (dataURL) {
    ...
    if (typeof dataURL === 'string') {
        ...
        let queryParams = decodeQuery(dataURL.substring(Math.min(dataURL.length, questionMarkInd)));
...
const validatePriorityNode = function (priorityNode) {
    if (priorityNode.isLeafNode()) {
        ...
        assert(typeof val === 'string' ||
            typeof val === 'number' ||
            (typeof val === 'object' &&
                contains(val, '.sv')), 'Priority must be a string or number.');
    }
    else {
...
class IndexMap {
    ...
    removeFromIndexes(namedNode, existingChildren) {
        const newIndexes = map(this.indexes_, function (indexedChildren) {
            if (indexedChildren === fallbackObject) {
 ...
 */
const resolveDeferredValue = function (value, serverValues) {
 ...
 * limitations under the License.
 */
class EmulatorAuthToken {
    constructor(accessToken) {
        this.accessToken = accessToken;
    }
}
class EmulatorAuthTokenProvider {
    constructor(app_) {
        this.app_ = app_;
    }
    getToken(forceRefresh) {
    ...
    removeTokenChangeListener(listener) { }
    notifyForInvalidToken() {
        let errorMessage = 'Database emulator unexpectedly rejected fake "owner" credentials.';
        warn(errorMessage);
    }
}
 ...
 */
class Repo {
    constructor(repoInfo_, forceRestClient, app) {
        this.repoInfo_ = repoInfo_;
        this.app = app;
...
function registerDatabase(instance) {
    ...
    const namespace = instance.INTERNAL.registerService('database', (app, unused, url) => RepoManager.getInstance().databaseFromApp(app, url), 
