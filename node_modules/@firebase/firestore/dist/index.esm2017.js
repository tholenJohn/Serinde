    ...
    addMutationBatch(transaction, localWriteTime, baseMutations, mutations) {
        ...
        return mutationStore.add({}).next(batchId => {
            ...
            for (const mutation of mutations) {
                ...
                promises.push(this.indexManager.addToCollectionParentIndex(transaction, mutation.key.path.popLast()));
            }
...
class SimpleDb {
    ...
    runTransaction(mode, objectStores, transactionFn) {
...
class IndexedDbQueryCache {
    ...
    addQueryData(transaction, queryData) {
    ...
    updateQueryData(transaction, queryData) {
        return this.saveQueryData(transaction, queryData);
    }
    removeQueryData(transaction, queryData) {
    ...
    saveQueryData(transaction, queryData) {
     ...
     */
    updateMetadataFromQueryData(queryData, metadata) {
        let updated = false;
    ...
    getQueryDataForTarget(transaction, targetId) {
        return targetsStore(transaction)
            .get(targetId)
            .next(found => {
            if (found) {
...
class Document extends MaybeDocument {
    ...
    getProtoField(path) {
        ...
        let protoValue = this.proto.fields[path.firstSegment()];
 ...
 * limitations under the License.
 */
const REMOTE_DOCUMENT_CHANGE_MISSING_ERR_MSG = 'The remote document changelog no longer contains all changes for all ' +
...
class IndexedDbRemoteDocumentCache {
     ...
     * cache.
     */
    constructor(serializer, indexManager, keepDocumentChangeLog) {
        this.serializer = serializer;
        this.indexManager = indexManager;
    ...
    get lastProcessedDocumentChangeId() {
    ...
    start(transaction) {
        const store = SimpleDb.getStore(transaction, DbRemoteDocumentChanges.store);
    ...
    addEntry(transaction, key, doc) {
        ...
        return documentStore.put(dbKey(key), doc).next(() => {
            this.indexManager.addToCollectionParentIndex(transaction, key.path.popLast());
        });
    }
    /**
     ...
     * cache's metadata.
     */
    updateMetadata(transaction, changedKeys, sizeDelta) {
        ...
        return changesStore
            ...
            if (firstIteration) {
                ...
                if (this._lastProcessedDocumentChangeId + 1 !== documentChange.id) {
                    ...
                    // rejection.
                    return this.synchronizeLastDocumentChangeId(changesStore).next(() => PersistencePromise.reject(new FirestoreError(Code.DATA_LOSS, REMOTE_DOCUMENT_CHANGE_MISSING_ERR_MSG)));
                }
            }
            changedKeys = changedKeys.unionWith(this.serializer.fromDbResourcePaths(documentChange.changes));
            this._lastProcessedDocumentChangeId = documentChange.id;
        })
            .next(() => {
            const documentPromises = [];
            changedKeys.forEach(key => {
                documentPromises.push(this.getEntry(transaction, key).next(maybeDocument => {
                    const doc = maybeDocument ||
     ...
     * (inclusive).
     */
    removeDocumentChangesThroughChangeId(transaction, changeId) {
        const range = IDBKeyRange.upperBound(changeId);
        return documentChangesStore(transaction).delete(range);
    }
    synchronizeLastDocumentChangeId(documentChangesStore) {
        ...
        this._lastProcessedDocumentChangeId = 0;
        return documentChangesStore.iterate({ keysOnly: true, reverse: true }, (key, value, control) => {
...
IndexedDbRemoteDocumentCache.RemoteDocumentChangeBuffer = class extends RemoteDocumentChangeBuffer {
    ...
    applyChanges(transaction) {
        ...
        this.changes.forEach((key, maybeDocument) => {
            ...
            if (maybeDocument) {
                const doc = this.documentCache.serializer.toDbRemoteDocument(maybeDocument);
...
class IndexedDbPersistence {
    ...
    startRemoteDocumentCache() {
        return this.simpleDb.runTransaction('readonly', ALL_STORES, txn => this.remoteDocumentCache.start(txn));
    }
    ...
    updateClientMetadataAndTryBecomePrimary() {
        return this.simpleDb.runTransaction('readwrite', ALL_STORES, txn => {
            ...
            return metadataStore
                .put(new DbClientMetadata(this.clientId, Date.now(), this.networkEnabled, this.inForeground, this.remoteDocumentCache.lastProcessedDocumentChangeId))
                .next(() => {
                if (this.isPrimary) {
                ...
                if (wasPrimary !== this.isPrimary) {
                    this.queue.enqueueAndForget(() => this.primaryStateListener(this.isPrimary));
                }
                if (wasPrimary && !this.isPrimary) {
    ...
    async maybeGarbageCollectMultiClientState() {
        if (this.isPrimary &&
            ...
            let activeClients;
            let inactiveClients = [];
            await this.runTransaction('maybeGarbageCollectMultiClientState', 'readwrite-primary', txn => {
                const metadataStore = IndexedDbPersistence.getStore(txn, DbClientMetadata.store);
                return metadataStore
                    .loadAll()
                    .next(existingClients => {
                    activeClients = this.filterActiveClients(existingClients, MAX_CLIENT_AGE_MS);
                    inactiveClients = existingClients.filter(client => activeClients.indexOf(client) === -1);
                })
                    .next(() => 
                // Delete metadata for clients that are no longer considered active.
                PersistencePromise.forEach(inactiveClients, (inactiveClient) => metadataStore.delete(inactiveClient.clientId)))
                    .next(() => {
                    ...
                    // `lastProcessedDocumentChangeId`.
                    activeClients = activeClients.filter(client => client.clientId !== this.clientId);
                    if (activeClients.length > 0) {
                        const processedChangeIds = activeClients.map(client => client.lastProcessedDocumentChangeId || 0);
                        const oldestChangeId = Math.min(...processedChangeIds);
                        return this.remoteDocumentCache.removeDocumentChangesThroughChangeId(txn, oldestChangeId);
                    }
                });
            });
    ...
    async shutdown() {
        ...
        await this.simpleDb.runTransaction('readwrite', [DbPrimaryClient.store, DbClientMetadata.store], txn => {
    ...
    getActiveClients() {
        return this.simpleDb.runTransaction('readonly', [DbClientMetadata.store], txn => {
            return clientMetadataStore(txn)
                .loadAll()
    ...
    runTransaction(action, mode, transactionOperation) {
        ...
        return this.simpleDb.runTransaction(mode === 'readonly' ? 'readonly' : 'readwrite', ALL_STORES, simpleDbTxn => {
            if (mode === 'readwrite-primary') {
                ...
                return this.verifyPrimaryLease(simpleDbTxn)
                    ...
                    return transactionOperation(new IndexedDbTransaction(simpleDbTxn, this.listenSequence.next()));
                })
                    .next(result => {
                ...
                return this.verifyAllowTabSynchronization(simpleDbTxn).next(() => transactionOperation(new IndexedDbTransaction(simpleDbTxn, this.listenSequence.next())));
            }
        });
    }
    /**
    ...
    getDocumentsMatchingCollectionGroupQuery(transaction, query) {
        ...
        return this.indexManager
            ...
            return PersistencePromise.forEach(parents, (parent) => {
                ...
                return this.getDocumentsMatchingCollectionQuery(transaction, collectionQuery).next(r => {
                    r.forEach((key, doc) => {
...
class LocalStore {
    ...
    handleUserChange(user) {
        return this.persistence.runTransaction('Handle user change', 'readonly', txn => {
    ...
    localWrite(mutations) {
        ...
        return this.persistence.runTransaction('Locally write mutations', 'readwrite', txn => {
    ...
    getLastRemoteSnapshotVersion() {
        return this.persistence.runTransaction('Get last remote snapshot version', 'readonly', txn => this.queryCache.getLastRemoteSnapshotVersion(txn));
    }
    /**
    ...
    applyRemoteEvent(remoteEvent) {
        ...
        return this.persistence.runTransaction('Apply remote event', 'readwrite-primary', txn => {
            const promises = [];
            forEachNumber(remoteEvent.targetChanges, (targetId, change) => {
                ...
                promises.push(this.queryCache
                    ...
                    .next(() => {
                    return this.queryCache.addMatchingKeys(txn, change.addedDocuments, targetId);
                }));
                const resumeToken = change.resumeToken;
                // Update the resume token if the change includes one.
                if (resumeToken.length > 0) {
                    const newQueryData = oldQueryData.withResumeToken(resumeToken, snapshotVersion);
                    ...
                    if (LocalStore.shouldPersistQueryData(oldQueryData, newQueryData, change)) {
                        promises.push(this.queryCache.updateQueryData(txn, newQueryData));
                    }
                }
            });
            let changedDocs = maybeDocumentMap();
            ...
            if (!snapshotVersion.isEqual(SnapshotVersion.MIN)) {
                const updateRemoteVersion = this.queryCache
                    ...
                    assert(snapshotVersion.compareTo(lastRemoteSnapshotVersion) >= 0, 'Watch stream reverted to previous snapshot?? ' +
                        ...
                        ' < ' +
                        lastRemoteSnapshotVersion);
                    return this.queryCache.setTargetsMetadata(txn, txn.currentSequenceNumber, snapshotVersion);
                });
     ...
     * too frequent.
     */
    static shouldPersistQueryData(oldQueryData, newQueryData, change) {
        assert(newQueryData.resumeToken.length > 0, 'Attempted to persist query data with no resume token');
        ...
        // closing.
        const timeDelta = newQueryData.snapshotVersion.toMicroseconds() -
    ...
    notifyLocalViewChanges(viewChanges) {
        return this.persistence.runTransaction('notifyLocalViewChanges', 'readwrite', txn => {
     ...
     */
    allocateQuery(query) {
        return this.persistence.runTransaction('Allocate query', 'readwrite', txn => {
            let queryData;
            return this.queryCache
                ...
                .next(() => {
                assert(!this.queryDataByTarget[queryData.targetId], 'Tried to allocate an already allocated query: ' + query);
    ...
    releaseQuery(query, keepPersistedQueryData) {
        ...
        return this.persistence.runTransaction('Release query', mode, txn => {
            return this.queryCache
                ...
                if (!keepPersistedQueryData) {
                    return PersistencePromise.forEach(removed, (key) => this.persistence.referenceDelegate.removeReference(txn, key)).next(() => this.persistence.referenceDelegate.removeTarget(txn, cachedQueryData));
                }
                else {
                    return PersistencePromise.resolve();
                }
    ...
    remoteDocumentKeys(targetId) {
        return this.persistence.runTransaction('Remote document keys', 'readonly', txn => {
    ...
    collectGarbage(garbageCollector) {
        return this.persistence.runTransaction('Collect garbage', 'readwrite-primary', txn => garbageCollector.collect(txn, this.queryDataByTarget));
    }
    // PORTING NOTE: Multi-tab only.
    getQueryForTarget(targetId) {
        if (this.queryDataByTarget[targetId]) {
            return Promise.resolve(this.queryDataByTarget[targetId].query);
        }
        else {
            return this.persistence.runTransaction('Get query data', 'readonly', txn => {
                return this.queryCache
    ...
    // PORTING NOTE: Multi-tab only.
    getNewDocumentChanges() {
        return this.persistence.runTransaction('Get new document changes', 'readonly', txn => {
...
class MemoryRemoteDocumentCache {
    ...
    getDocumentsMatchingQuery(transaction, query) {
        ...
        while (iterator.hasNext()) {
            const { key, value: { maybeDocument } } = iterator.getNext();
...
class WatchChangeAggregator {
     ...
     */
    queryDataForActiveTarget(targetId) {
...
class RemoteStore {
    ...
    async onWatchStreamOpen() {
        forEachNumber(this.listenTargets, (targetId, queryData) => {
            this.sendWatchRequest(queryData);
        });
    }
    ...
    raiseWatchSnapshot(snapshotVersion) {
        ...
        forEachNumber(remoteEvent.targetChanges, (targetId, change) => {
            if (change.resumeToken.length > 0) {
                ...
                if (queryData) {
                    this.listenTargets[targetId] = queryData.withResumeToken(change.resumeToken, snapshotVersion);
                }
            }
        });
        ...
        remoteEvent.targetMismatches.forEach(targetId => {
            ...
            // state.
            this.listenTargets[targetId] = queryData.withResumeToken(emptyByteString(), queryData.snapshotVersion);
            ...
            const requestQueryData = new QueryData(queryData.query, targetId, QueryPurpose.ExistenceFilterMismatch, queryData.sequenceNumber);
...
class Query {
     ...
     */
    constructor(path, collectionGroup = null, explicitOrderBy = [], filters = [], limit = null, startAt = null, endAt = null) {
        this.path = path;
...
class JsonProtoSerializer {
    ...
    toListenRequestLabels(queryData) {
...
class LocalViewChanges {
    constructor(targetId, addedKeys, removedKeys) {
        this.targetId = targetId;
...
class View {
    ...
    computeDocChanges(docChanges, previousChanges) {
        ...
        const lastDocInLimit = this.query.hasLimit() && oldDocumentSet.size === this.query.limit
            ? oldDocumentSet.last()
            : null;
     ...
     */
    // PORTING NOTE: Multi-tab only.
    synchronizeWithPersistedState(localDocs, remoteKeys) {
...
class SyncEngine {
    ...
    async listen(query) {
        ...
        if (queryView) {
            ...
            const status = this.sharedClientState.addLocalQueryTarget(queryData.targetId);
            targetId = queryData.targetId;
            viewSnapshot = await this.initializeViewAndComputeSnapshot(queryData, status === 'current');
            if (this.isPrimary) {
     ...
     * snapshot.
     */
    async initializeViewAndComputeSnapshot(queryData, current) {
        const query = queryData.query;
        const docs = await this.localStore.executeQuery(query);
        const remoteKeys = await this.localStore.remoteDocumentKeys(queryData.targetId);
        const view = new View(query, remoteKeys);
        const viewDocChanges = view.computeDocChanges(docs);
        const synthesizedTargetChange = TargetChange.createSynthesizedTargetChangeForCurrentChange(queryData.targetId, current && this.onlineState !== OnlineState.Offline);
    ...
    async synchronizeViewAndComputeSnapshot(queryView) {
        ...
        const remoteKeys = await this.localStore.remoteDocumentKeys(queryView.targetId);
        const viewSnapshot = queryView.view.synchronizeWithPersistedState(docs, remoteKeys);
        if (this.isPrimary) {
    ...
    async applyPrimaryState(isPrimary) {
        if (isPrimary === true && this.isPrimary !== true) {
            ...
            forEachNumber(this.queryViewsByTarget, (targetId, queryView) => {
    ...
    async synchronizeQueryViewsAndRaiseSnapshots(targets) {
        ...
        for (const targetId of targets) {
            ...
            if (queryView) {
                ...
                await this.initializeViewAndComputeSnapshot(queryData, 
                /*current=*/ false);
            }
                ...
                case 'not-current': {
                    try {
                        ...
                        if (isDocumentChangeMissingError(error)) {
                            const activeTargets = [];
                            forEachNumber(this.queryViewsByTarget, target => activeTargets.push(target));
                            await this.synchronizeQueryViewsAndRaiseSnapshots(activeTargets);
                        }
                        else {
                            throw error;
...
class WebStorageSharedClientState {
    ...
    async start() {
        ...
        for (const clientId of existingClients) {
            ...
            const storageItem = this.getItem(this.toWebStorageClientStateKey(clientId));
            if (storageItem) {
    ...
    addLocalQueryTarget(targetId) {
        ...
        if (this.isActiveQueryTarget(targetId)) {
            const storageItem = this.storage.getItem(this.toWebStorageQueryTargetMetadataKey(targetId));
            if (storageItem) {
    ...
    toWebStorageClientStateKey(clientId) {
    ...
    toWebStorageQueryTargetMetadataKey(targetId) {
        return `${QUERY_TARGET_KEY_PREFIX}_${this.persistenceKey}_${targetId}`;
    }
    /** Assembles the key for a mutation batch in WebStorage */
    toWebStorageMutationBatchKey(batchId) {
...
class UserDataConverter {
     ...
     * bound).
     */
    parseQueryValue(methodName, input) {
...
class DocumentReference {
    constructor(_key, firestore) {
        this._key = _key;
        this.firestore = firestore;
        this._firestoreClient = this.firestore.ensureClientConfigured();
    }
    static forPath(path, firestore) {
        if (path.length % 2 !== 0) {
        ...
        const asyncObserver = new AsyncObserver({
            next: snapshot => {
                if (observer.next) {
                    ...
                    observer.next(new DocumentSnapshot(this.firestore, this._key, doc, snapshot.fromCache, snapshot.hasPendingWrites));
                }
            },
            error: errHandler
        });
...
class DocumentSnapshot {
    constructor(_firestore, _key, _document, _fromCache, _hasPendingWrites) {
        this._firestore = _firestore;
        this._key = _key;
...
class QuerySnapshot {
    constructor(_firestore, _originalQuery, _snapshot) {
        this._firestore = _firestore;
...
function changesFromSnapshot(firestore, includeMetadataChanges, snapshot) {
    if (snapshot.oldDocs.isEmpty()) {
        ...
        return snapshot.docChanges.map(change => {
            const doc = new QueryDocumentSnapshot(firestore, change.doc.key, change.doc, snapshot.fromCache, snapshot.mutatedKeys.has(change.doc.key));
...
function configureForFirebase(firebase) {
    firebase.INTERNAL.registerService('firestore', (app) => new Firestore(app), shallowCopy(firestoreNamespace));
}
    ...
    openStream(rpcName, token) {
        ...
        unguardedEventListen(WebChannel.EventType.MESSAGE, msg => {
            if (!closed) {
                ...
                const msgDataAsAny = msgData;
                const error = msgDataAsAny.error || (msgDataAsAny[0] && msgDataAsAny[0].error);
                if (error) {
